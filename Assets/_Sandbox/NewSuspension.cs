...
public class NewSuspension : MonoBehaviour
{
    ...
    void ApplySuspensionForce()
    {
        if (wheel.grounded)
        {
            ...
            {
                Vector3 appliedSuspensionForce = (leaningForce ? Vector3.Lerp(upDir, vp.norm.forward, Mathf.Abs(Mathf.Pow(Vector3.Dot(vp.norm.forward, vp.transform.up), 5))) : vp.norm.forward) *
