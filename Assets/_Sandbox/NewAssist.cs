...
public class NewAssist : MonoBehaviour
{
    ...
    void ApplySpinAssist()
    {
        ...
        if (straightenAssist && vp.steerInput == 0 && Mathf.Abs(rightVelDot) < 0.1f && vp.rb.linearVelocity.sqrMagnitude > 5)
        {
            rb.AddRelativeTorque(
    ...
    void ApplyDriftPush()
    {
        float pushFactor = (vp.accelAxisIsBrake ? vp.accelInput : vp.accelInput - vp.brakeInput) * Mathf.Abs(vp.localVelocity.x) * driftPush * groundedFactor * (1 - Mathf.Abs(Vector3.Dot(vp.transform.forward, rb.linearVelocity.normalized)));
